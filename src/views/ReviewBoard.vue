<template>
  <div>
    ...
    <div class="review-list">
      <h2>여행 리뷰</h2>
      <button @click="openModal" class="write-review-button">리뷰 작성하기</button>
      <!-- 모달 -->
      <div class="modal" v-if="isModalOpen">
        <div class="modal-content">
          <span class="close" @click="closeModal">&times;</span>
          <h3>리뷰 작성</h3>
          <textarea v-model="newReviewTitle" placeholder="제목" class="input-title"></textarea>
          <textarea v-model="newReviewContent" placeholder="내용" class="input-content"></textarea>
...
const deleteReview = (reviewId) => {
  axios.delete(`http://localhost:8080/post/${reviewId}`)
    .then(() => {
...
const fetchReviews = () => {
  axios.get(`http://localhost:8080/post/spot/${id}`)
    .then(response => {
