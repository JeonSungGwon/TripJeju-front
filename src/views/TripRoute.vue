<template>
    ...
    <div id="container">
      <div id="mainContainer">
        <div class="search-container">
          ...
          <input type="text" v-model="searchTerm" placeholder="여행 경로를 입력해주세요" class="search-input" />
        </div>
        <div class="row">
          <div class="col-lg-3 col-md-6 mb-4" v-for="route in filteredRoutes" :key="route.id">
            <div class="single-team-member" @click="openModal(route.id)">
              <div class="team-member-img">
                <img :src="route.img" alt="Route Thumbnail" height="300" width="281">
              </div>
      ...
      <div v-if="selectedRouteId" class="modal-overlay" @click.self="closeModal">
  ...
  const fetchRoutes = async () => {
    ...
    try {
      const { data: fetchedRoutes } = await axios.get(`http://localhost:8080/travel-route`, {
  ...
  const filteredRoutes = computed(() => {
    return routes.value.filter(route => {
      return route.routeName.toLowerCase().includes(searchTerm.value.toLowerCase());
    });
  });
