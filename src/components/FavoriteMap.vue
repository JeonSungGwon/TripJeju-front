...
import favoriteMarkerImage from '/assets/img/favoriteMarker.png';
import favoriteOnImage from '/assets/img/favoriteOn.png';
import favoriteOffImage from '/assets/img/favoriteOff.png';
      ...
      if (this.spots && this.spots.length > 0) {
        this.spots.forEach((spot) => {
          ...
          const contentString = [
            ...
            `       </div>`,
            `   </p>`,
            `   <img src="${isFavorite ? this.favoriteOnImage : this.favoriteOffImage}" class="favorite-button" style="width: 32px; height: 32px; cursor: pointer;" data-spot-id="${spot.id}" />`,
            `</div>`,
          ].join("");
    ...
    addFavorite(placeId, element) {
      axios
        ...
        .then(() => {
          ...
          element.src = this.favoriteOnImage;
          this.loadFavorites();
        })
    ...
    removeFavorite(favoriteId, element) {
      axios
        ...
        .then(() => {
          ...
          element.src = this.favoriteOffImage;
          this.loadFavorites();
        })
        .catch((error) =>
